...
import type { GreetingCelebrityFilter } from "@/lib/studio/greeting-funnel-data";
import { GREETING_CELEBRITIES } from "@/lib/studio/greeting-funnel-data";

const FILTERS: GreetingCelebrityFilter[] = ["All", "Music", "Entertainment", "Sports", "TV", "Business"];
...
export function SelectGreetingCelebrity({ selectedId, onSelect }: SelectGreetingCelebrityProps) {
  const [filter, setFilter] = useState<GreetingCelebrityFilter>("All");
  const list = useMemo(() => {
    if (filter === "All") return GREETING_CELEBRITIES;
    return GREETING_CELEBRITIES.filter((c) => c.filter === filter);
  }, [filter]);
        ...
        {list.map((c) => {
          ...
          return (
            ...
            >
              ...
              <p className="mt-1 text-xs text-white/35">Delivery: {c.deliveryLabel}</p>
            </button>
          );
        })}
      </div>
