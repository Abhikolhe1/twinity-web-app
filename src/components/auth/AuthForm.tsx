...
export default function AuthForm({ mode }: AuthFormProps) {
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async ({ access_token }) => {
      ...
      setError('')
      try {
        const res = await authApi.googleAuth(access_token)
        setToken(res.token)
        ...
        <div className="w-full max-w-md relative z-10">
          ...
          <div className="mb-6">
            ...
            <button
              ...
              onClick={() => googleLogin()}
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl bg-white border border-brand-purple/18 text-sm text-content-secondary hover:bg-surface-subtle hover:text-content-primary hover:border-brand-purple/30 transition-all shadow-card disabled:opacity-60 disabled:cursor-not-allowed"
            >
